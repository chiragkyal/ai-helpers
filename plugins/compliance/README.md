...
Jira/PR features require an Atlassian MCP server (or `jira-cli`) and, for PR creation, an authenticated `gh` CLI. These are optional — direct CVE analysis (with `--repo=`) works without them. Running on a hosted Remote Workspace where the worker has no Jira credentials of its own? See [`reference/chai-bot-rws.md`](reference/chai-bot-rws.md) for the coordinator/worker split.

## Repository Resolution

The skill always analyzes a **cloned repository** (`REPO_DIR`, under `.work/compliance/analyze-cve/repos/`) — it does not analyze whatever directory it happens to be invoked from. Resolution order (see [Phase 0.7](skills/analyze-cve/reference/implementation.md#phase-07-repository-resolution-and-cloning) for full detail):
...
```text
/compliance:analyze-cve CVE-2024-45338 --repo=https://github.com/openshift/hypershift
```
**Result:**
...
Running this plugin's skill inside a hosted Remote Workspace pod (e.g. Chai Bot)? Worker pods there typically hold no Jira credentials, so the Jira steps (ticket fetch, JQL search, comment/label posting) must be done by the *coordinator*, not inside the worker. See [`reference/chai-bot-rws.md`](reference/chai-bot-rws.md) for the coordinator/worker adapter pattern.
